...
축구를 보다 보면 분명 골이 들어갔는데 심판이 손을 들고 취소하는 장면, 한 번쯤 보셨을 거예요. 십중팔구 **오프사이드**입니다. 축구 규칙 중 가장 헷갈리는 게 바로 이 오프사이드인데, 핵심만 잡으면 의외로 단순합니다. 이 글에서는 축구 오프사이드 규칙을 처음 보는 분도 이해할 수 있게, 위치의 조건부터 실제 반칙이 되는 순간, 그리고 예외까지 차근차근 풀어드릴게요.
...
여기가 가장 중요한 포인트입니다. **오프사이드 위치에 서 있는 것만으로는 반칙이 아닙니다.** 그 위치에서 실제로 플레이에 관여할 때 비로소 반칙이 돼요. 관여한다는 건 이런 경우입니다.
...
오프사이드는 **공을 받는 순간이 아니라, 같은 편이 공을 차는(터치하는) 순간**의 위치로 판단합니다. 이게 중계 보면서 제일 헷갈리는 부분이에요. 예를 들어 패스하는 순간엔 수비 라인보다 뒤(정당한 위치)에 있던 선수가, 빠르게 뛰어나가 공을 받으면 오프사이드가 아닙니다. 받는 위치가 앞서 있어도, 기준은 '패스가 출발한 그 순간'이니까요.
...
정리하면, 오프사이드는 ① 위치 조건을 만족하고 ② 그 위치에서 실제로 플레이에 관여할 때, ③ 같은 편이 공을 차는 순간을 기준으로 성립합니다. 이 셋만 기억하면 경기 보는 재미가 확 달라져요. 스포츠킹덤24가 어려운 규칙도 쉽게 풀어드릴게요. 처음 오셨다면 [스포츠킹덤24 소개](/welcome/)도 둘러보세요. ⚽
